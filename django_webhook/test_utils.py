"""
Test support (spec E4).

On-commit dispatch (spec A1) makes naive assertions silently vacuous: a
deferred callback never runs inside a test's transaction, so a test that asserts
"no event was sent" passes whether or not the code is correct. This module ships
the correct primitives so each project does not have to rediscover the trap:
