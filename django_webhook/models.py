...
class WebhookEventQuerySet(models.QuerySet):
    ...
    def resend(self):
        """
        Re-enqueue every delivery in this queryset. Returns the number of
        deliveries that were re-fired.
        """
        count = 0
        for event in self:
            event.resend()  # type: ignore[attr-defined]
            count += 1
        return count
...
class WebhookEvent(models.Model):
    ...
    def resend(self):
        ...
        fires the delivery task against the stored payload (spec B4).
        """
...
def sync_topics(*, prune=True):
    ...
    e.g. from the ``webhook_sync_topics`` management command (spec E2).
