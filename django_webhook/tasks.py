...
def fire_webhook(  # pylint: disable=too-many-arguments,too-many-positional-arguments
    ...
    then retry with backoff up to ``MAX_RETRIES`` (spec B1). The row always
    reaches a terminal recorded state (spec B3).
    """
...
def _update_event(webhook_event_id, *, status, error=None):
    ...
    # Only a delivery failure writes the error field. A successful delivery
    # leaves any production-time degradation note (e.g. an unserializable value
    # coerced by the fallback encoder) intact so the loss is never hidden.
...
def resend_webhook_event(webhook_event_id):
    """
    Public programmatic re-send for a single recorded delivery (spec B4).
    Returns ``True`` if the delivery was re-enqueued.
    """
...
def resend_webhook_events(queryset):
    """
    Re-send a filtered batch of recorded deliveries (spec B4). Accepts any
...
def clear_webhook_events():
    ...
    timer (spec B5). Failed deliveries are retained forever unless
    ...
    if succeeded_days is not None:
        ...
        # PENDING rows are in-flight; only purge terminal succeeded rows here.
