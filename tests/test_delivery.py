"""
Delivery reliability: every failure mode retries and records (B1), requests
carry a timeout (B2), and deliveries reach a terminal recorded status (B3).
"""
...
def test_all_failure_modes_retry_and_reach_terminal_failure(
    ...
    # record the failure and retry, ending in a terminal FAILURE (B1, B3) —
...
def test_request_carries_configured_timeout(settings, mocker):
    ...
    # with a non-infinite default (B2).
    settings.DJANGO_WEBHOOK = dict(
...
def test_inactive_webhook_reaches_terminal_state(settings, responses):
    ...
    # leave PENDING so it is not mistaken for in-flight (B3).
    settings.DJANGO_WEBHOOK = dict(MODELS=["tests.User"], USE_CACHE=False)
